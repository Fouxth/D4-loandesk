...
async function handleSummary(userId: string, replyToken: string, tenantId: string) {
  ...
  const [[payments], [loans], [expenses]] = await Promise.all([
    ...
    sql`SELECT COALESCE(SUM(principal::numeric), 0) as total FROM loans WHERE tenant_id = ${tenantId} AND (is_pawn IS NOT TRUE) AND (start_date = ${today} OR DATE(created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok') = ${today})`,
